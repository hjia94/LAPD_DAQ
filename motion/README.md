...
- Supported: stationary, XY grid, and XYZ grid motion.
...
`PositionManager` is used by the `acquisition` runner to create planned
positions, initialize HDF5 position groups, initialize direct motor control, and
record achieved positions.
...
Those settings are preserved for reference, but 45-degree acquisition is not
run. Use the known pre-refactor hardware PC workflow until 45-degree support is
explicitly migrated and mock-tested.
